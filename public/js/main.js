...
function setupEventListeners() {
  ...
  document
    .querySelector("#go-to-missions-btn")
  ...
  // Leave Team Button
  document
    .querySelector("#leave-team-btn")
    .addEventListener("click", leaveTeam);
  ...
  document
    .querySelector("#cancel-upload-btn")
...
async function getTeam() {
  ...
  if (saved) {
    try {
      ...
      if (myTeam) {
        const res = await api("POST", serverUrl + "/api/teams", {
          name: myTeam.name,
        });
        if (res.error)
          return showError(
            res.error === "Team name already taken"
...
function connectWS() {
  ...
  console.log(
    `Connecting to WebSocket at ${proto}://${location.hostname}:${serverPort}`,
  );
...
function leaveTeam() {
  if (
    !confirm(
      "Wil je een ander team aanmaken? Je verliest je huidige voortgang niet, maar je koppeling verdwijnt.",
    )
  )
    return;
  myTeam = null;
...
function renderMissions() {
  ...
  document.querySelector("#missions-progress").textContent =
...
async function toggleMission(missionId) {
  ...
  const res = await api(
    "POST",
    serverUrl + `/api/teams/${myTeam.id}/missions/${missionId}`,
  );
  ...
  showToast(
    done ? `+${m.pts} punten! 🎉` : "Opdracht ongedaan gemaakt",
    done ? "success" : "",
  );
}
...
async function doUpload(activityType = "mission_upload") {
  ...
  console.log("Uploading file for mission", fd);

  try {
    const res = await fetch(serverUrl + `/api/teams/${myTeam.id}/upload`, {
      method: "POST",
      body: fd,
    });
  ...
  if (
    ...
    optionsHtml = `
                <div class="quiz-image-wrap">
                    <img id="quiz-question-img" src="${escHtml(activeQuestion.imageUrl)}" alt="Question image" style="max-width:100%;border-radius:var(--radius-sm);margin-bottom:16px">
                    <input class="input" id="file-input" type="file" accept="image/*"
                        placeholder="Kies een afbeelding..." style="display:none">
                    <button class="btn" onclick="document.querySelector('#file-input').click()" style="width:100%;margin-bottom:8px">📷 Kies afbeelding</button>
                    <button class="btn" id="quiz-image-submit-btn" onclick="submitImageAnswer()" style="width:100%">Antwoord versturen →</button>
                </div>`;
...
function renderQuizResult(data) {
  ...
  // Find my result
  const myResult = myTeam
    ? data.results.find((r) => r.teamId === myTeam.id)
    : null;
